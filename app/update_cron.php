...
if ($client -> error) {
  ...
  if ($result) {
    ...
    foreach ( $result as $res ) {
      ...
      if (db :: table ( 'my_sorting_criteria' ) -> where ( 'criteria', $res -> gender ) 
        ...
        if ($res -> gender and (strlen ( $res -> gender ) > 0))
          db :: table ( 'my_sorting_criteria' ) -> insert ( [ 'criteria' => $res -> gender,'gender' => 1 ] );
      }
      
      // add the secondary fields
      foreach ( $res -> secondary_fields as $key => $value ) {
        db :: table ( 'my_secondary_fields' ) -> updateOrInsert ( [ 'aid' => $res -> aid,'category_id' => $key ], [ 'aid' => $res -> aid,'category_id' => $key,'name' => $value ] );
        // add the secondary fields if they aren't there
        if (db :: table ( 'my_sorting_criteria' ) -> where ( 'field_id', $key ) 
          -> doesntExist ()) {
          // add the criteria
          db :: table ( 'my_sorting_criteria' ) -> insert ( [ 'criteria' => $value,'field_id' => $key ] );
        }
      }
      // now the conferences
      foreach ( $res -> conferences as $key => $value ) {
        db :: table ( 'myconferences' ) -> updateOrInsert ( [ 'aid' => $res -> aid,'conference_id' => $key ], [ 'aid' => $res -> aid,'conference_id' => $key,'conference_name' => $value ] );
        if (db :: table ( 'my_sorting_criteria' ) -> where ( 'conference_id', $key ) 
          -> doesntExist ()) {
          // add the criteria
          db :: table ( 'my_sorting_criteria' ) -> insert ( [ 'criteria' => $value,'conference_id' => $key ] );
        }
      }
    }
    ...
    foreach ( $myaids as $key => $value ) {
      $kn ++;
      if ($value == 0) {
        db :: table ( 'myapplicants' ) -> where ( 'aid', $key ) 
          -> delete ();
        db :: table ( 'myconferences' ) -> where ( 'aid', $key ) 
          -> delete ();
        db :: table ( 'my_secondary_fields' ) -> where ( 'aid', $key ) 
          -> delete ();
        $k ++;
      }
    }
    ...
    print "Nothing returned fetching applicants\n";
  ...
  if ($result) {
    ...
    // print "counted ".count($myappids)." appliations\n";
    unset ( $myappids_stub );
    foreach ( $result as $res ) {
      if ($showdots)
        print ("*") ;
      $myappids [$res -> appid] = 1;
      db :: table ( 'myapplications' ) -> updateOrInsert ( [ 'appid' => $res -> appid ], [ 'appid' => $res -> appid,'aid' => $res -> aid,'posid' => $res -> posid,'appdate' => $res -> appdate,'changedate' => $res -> changedate,'adtitle' => $res -> adtitle ] );
      // mark the gender of the application
      $criteria_id = $u -> getGenderId ( $res -> aid );
      if (db :: table ( 'my_criteria_assignment' ) -> where ( [ [ 'criteria_id',$criteria_id ],[ 'appid',$res -> appid ] ] ) 
        -> doesntExist ()) {
        // add the criteria
        db :: table ( 'my_criteria_assignment' ) -> insert ( [ 'appid' => $res -> appid,'criteria_id' => $criteria_id ] );
      }
      // next add the secondary to mark the applications
      $fields = $u -> getFields ( $res -> aid );
      foreach ( $fields as $field ) {
        if (db :: table ( 'my_criteria_assignment' ) -> where ( [ [ 'criteria_id',$field ],[ 'appid',$res -> appid ] ] ) 
          -> doesntExist ()) {
          // add the criteria
          db :: table ( 'my_criteria_assignment' ) -> insert ( [ 'appid' => $res -> appid,'criteria_id' => $field ] );
        }
      }
      ...
      foreach ( $conferences as $conference ) {
        if (db :: table ( 'my_criteria_assignment' ) -> where ( [ [ 'criteria_id',$conference ],[ 'appid',$res -> appid ] ] ) 
          -> doesntExist ()) {
          // add the criteria
          db :: table ( 'my_criteria_assignment' ) -> insert ( [ 'appid' => $res -> appid,'criteria_id' => $conference ] );
        }
      }
      
      foreach ( $res -> questions as $question ) {
        db :: table ( 'myquestions' ) -> updateOrInsert ( [ 'appid' => $res -> appid,'question_id' => $question -> id ], [ 'appid' => $res -> appid,
        ...
        if ($question -> type_id != 'text') {
          if (db :: table ( 'my_sorting_criteria' ) -> where ( 'response_id', $question -> response_id ) 
            -> doesntExist ()) {
            // add the criteria
            db :: table ( 'my_sorting_criteria' ) -> insert ( [ 'criteria' => $question -> response_text,'response_id' => $question -> response_id,'posid' => $res -> posid ] );
          }
      ...
      if (count ( $res -> files ) > 0) {
        foreach ( $res -> files as $file ) {
          if ($file -> file_id > 0) {
            db :: table ( 'myfiles' ) -> updateOrInsert ( [ 'appid' => $res -> appid,'file_id' => $file -> file_id ], [ 'appid' => $res -> appid,
      ...
      foreach ( $res -> recommendations as $recommendation ) {
        db :: table ( 'myrecommendations' ) -> where ( [ 
        ] ) 
          -> updateOrInsert ( [ 'appid' => $res -> appid,'recid' => $recommendation -> recid ], [ 'appid' => $res -> appid,
    ...
    foreach ( $myappids as $key => $value ) {
      $kn ++;
      if ($value == 0) {
        db :: table ( 'myapplications' ) -> where ( 'appid', $key ) 
          -> delete ();
        db :: table ( 'my_criteria_assignment' ) -> where ( 'appid', $key ) 
          -> delete ();
        db :: table ( 'myquestions' ) -> where ( 'appid', $key ) 
          -> delete ();
        db :: table ( 'myfiles' ) -> where ( 'appid', $key ) 
          -> delete ();
        db :: table ( 'myrecommendations' ) -> where ( 'appid', $key ) 
          -> delete ();
        $k ++;
      }
    }
    ...
    print "Nothing returned fetching applications\n";
  ...
  if (! $cfg -> transfer_blobs)
    die ( "Ended without blob transfer as per config" );
  print "Processing files:\n";
  $user_files = db :: table ( 'myfiles' ) -> select ( 'file_id', 'blobid' ) 
    -> get ();
  $n = 0;
  foreach ( $user_files as $user_file ) {
    ...
    if (db :: table ( 'myblobs' ) -> where ( 'blobid', $user_file -> blobid ) 
      -> doesntExist ()) {
      ...
      // die("For debug");
      if (! $result -> {0} -> blobid or ($result -> {0} -> blobid == 0))
        continue;
      // print "\npassed continue\n";
      $n ++;
      // if($n > 2) die("ended for debug at 10");
      db :: table ( 'myblobs' ) -> insert ( [ 'blobid' => $result -> {0} -> blobid,
  ...
  $user_letters = db :: table ( 'myrecommendations' ) -> select ( 'letterid', 'blobid' ) 
    -> get ();
  $n = 0;
  foreach ( $user_letters as $user_letter ) {
    ...
    if (db :: table ( 'myblobs' ) -> where ( 'blobid', $user_letter -> blobid ) 
      -> doesntExist ()) {
  ...
  foreach ( $result as $res ) {
    db :: table ( 'my_placement_officers' ) -> updateOrInsert ( [ 'department' => $res -> department,'name' => $res -> name ], [ 'fname' => $res -> fname,'lname' => $res -> lname,'email' => $res -> email,'urlplacement' => $res -> urlplacement ] );
  }
